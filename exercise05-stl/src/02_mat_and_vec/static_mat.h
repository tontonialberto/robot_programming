...
namespace rp {
  ...
  public:
    ...
    inline  ThisType& operator*=(Scalar& scalar) {
      for (int i=0; i<Dimension; ++i)
        _values[i]*=scalar;
      return *this;
    }
    
    inline  ColVecType operator*(RowVecType& v) const {
      ColVecType ret;
      Scalar acc=Scalar(0);
    ...
    inline  ThisTypeTransposed transpose() const {
    ...
    inline  Mat_<Scalar, Rows, OtherCols> operator *(const Mat_<Scalar, Cols, OtherCols>& other) {
